...
int main(int argc,char** argv){
	...
	if(err>=0){
		...
		/*Set Kin{{{*/
		const Double_t Ep_Bin_Size = 0.001; //1 MeV per bin
		if(gAngle==21)  {
			Ep_Min = 2.700; Ep_Max = 3.300; }//600 Bins 
		else if(gAngle==23) {   
			Ep_Min = 2.650; Ep_Max = 3.300; } //680 Bins
		else if(gAngle==25) {   
			Ep_Min = 2.300; Ep_Max = 3.300; } //1000 Bins
		else if(gAngle==28) {   
			Ep_Min = 2.650; Ep_Max = 3.100; } //450 Bins
		else {
		...
		Double_t Delta_Theta = 0.05*TMath::RadToDeg() * 2.0;      Int_t Bin_Theta = 500;   Double_t Size_Theta = Delta_Theta/Bin_Theta;

		//Define Output file name
		ofstream outfile(Table_File.Data());
        if(gMinBin==0)
			outfile << Form("%5s %5s %10s %10s %6s %10s %6s %10s %10s %16s %16s %16s %16s %16s", "A","Z","Scatt_Ang","Ep","EBin","Theta","TBin", "Q2", "xbj", "xs_born","xs_qe","xs_dis","xs_rad","Tail")<<endl;
		...
		for(int j=gMinBin;  j<gMaxBin;   j++){
			...
			for(int i=0;  i<=Bin_Theta; i++){
                ...
                outfile << Form("%5d %5d %10.3f %10.7f %6d %10.7f %6d %10.7f %10.7f %16.6e %16.6e %16.6e %16.6e %16.6e", A,Z,Theta,mom,j,theta,i,Q2, Xbj, cs_born,cs_qe,cs_dis,cs_rad,tail)<<endl;
            if(!i%10)
                cerr << Form("%5d %5d %10.3f %10.7f %6d %10.7f %6d %10.7f %10.7f %16.6e %16.6e %16.6e %16.6e %16.6e", A,Z,Theta,mom,j,theta,i,Q2, Xbj, cs_born,cs_qe,cs_dis,cs_rad,tail)<<endl;
            }
		} 
		delete Event;
		outfile.close();
