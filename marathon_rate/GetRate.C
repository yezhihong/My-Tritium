...
/*GetRate{{{*/
void GetRate(const string spec, const string Model,  const string Com, double E0, double P0, double T0, const double I_Current, 
    ...
    //Empty Cell Thickness
    //Upstream Window:   0.0254cm x 2.84g/cm-3
    //Downstream Window: 0.0279cm x 2.84g/cm-3
    double Window_Length = 25.;//cm
    if(spec=="HRS")
        Window_Length = 15.0;//cm
    Beam_Current = I_Current*1e-6;//25uA
    const double Window_ArialDensity =  ( 0.072+0.079 );//g/cm-2, Al-2219 Alloy
    double Rad_Len_Window_Up = 0.072 / Get_Rad_Len(27, 13) *100.; //%
    double Rad_Len_Window_Dn = 0.079 / Get_Rad_Len(27, 13) *100.; //%
...
/*}}}*/
    ...
    for(int i=0; i<Nevnt; i++){
        ...
        if(xb<1.0){
            ...
            RL = 0.5 * Rad_Len_D2*Endcap_Cor_D2 + 5.0;
            wiser_all_sig_(&E0, &Ep, &Theta_Deg,&RL, &type, &sigma_wiser_d2);
            RL = 0.5 * Rad_Len_H3*Endcap_Cor_H3 + 5.0;
            wiser_all_sig_(&E0, &Ep, &Theta_Deg,&RL, &type, &sigma_wiser_h3);
            RL = 0.5 * Rad_Len_He3*Endcap_Cor_He3 + 5.0;
...
int main(){
    ...
    double P0 = 0; cout<<"---   Mom = "; cin >> P0;
    double T0 = 0; cout<<"--- Theta = "; cin >> T0;
    ...
    double pi_e[6];
    double xs_he3, xs_h3, xs_d2,xs_he3_avg, xs_h3_avg, xs_d2_avg, xbj_avg,W2_avg;
    ...
    double nu = E0 - P0;
    double SinSQ = pow(sin(T0/2.*Deg2Rad), 2);
    Q2 = 4.0 * E0 * P0 * SinSQ;
    ...
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("**    D2:  Total Rate = %6.2f Hz, e Rate = %6.2f Hz (%6.2f Hz good e,%4.1f hrs), Pi/E=%3.1f, N(ele)=%dK, N(tot)=%dK, time=%dmins \n", 
		    (rate_d2+rate_w*Endcap_Cor_D2)*(pi_e[0]+1), (rate_d2+rate_w*Endcap_Cor_D2), rate_d2, hour_d2,pi_e[0], 25, int((rate_d2+rate_w*Endcap_Cor_D2)*(pi_e[0]+1)*25.+0.5), int(25000./(rate_d2)/60.+0.5) );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("**    H3:  Total Rate = %6.2f Hz, e Rate = %6.2f Hz (%6.2f Hz good e,%4.1f hrs), Pi/E=%3.1f, N(ele)=%dK, N(tot)=%dK, time=%dmins \n", 
		    (rate_h3+rate_w*Endcap_Cor_H3)*(pi_e[1]+1), (rate_h3+rate_w*Endcap_Cor_H3), rate_h3, hour_h3,pi_e[1], 25, int((rate_h3+rate_w*Endcap_Cor_H3)*(pi_e[1]+1)*25.+0.5), int(25000./(rate_h3)/60.+0.5) );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("**   He3:  Total Rate = %6.2f Hz, e Rate = %6.2f Hz (%6.2f Hz good e,%4.1f hrs), Pi/E=%3.1f, N(ele)=%dK, N(tot)=%dK, time=%dmins \n", 
		    (rate_he3+rate_w*Endcap_Cor_He3)*(pi_e[2]+1), (rate_he3+rate_w*Endcap_Cor_He3), rate_he3, hour_he3,pi_e[2], 25, int((rate_he3+rate_w*Endcap_Cor_He3)*(pi_e[2]+1)*25.+0.5), int(25000./(rate_he3)/60.+0.5) );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("** Empty:  Total Rate = %6.2f Hz, e Rate = %6.2f, Pi/E=%3.1f, N(ele)=%dK, N(tot)=%dK, time=%dmins  \n",
		    (rate_w)*(pi_e[3]+1), rate_w,pi_e[3], 25, int((rate_w)*(pi_e[3]+1)*25.+0.5), int(25000./(rate_w)/60.+0.5) );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("**Optics:  Total Rate = %6.2f Hz, e Rate = %6.2f, Pi/E=%3.1f  \n",(rate_c12)*(pi_e[4]+1),  rate_c12,pi_e[4] );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    printf("** Dummy:  Total Rate = %6.2f Hz, e Rate = %6.2f Pi/E=%3.1f  \n", (rate_al)*(pi_e[5]+1), rate_al,pi_e[5] );
    cout<<"-----------------------------------------------------------------------------------------"<<endl;
    cout<<endl;
